// Axel '0vercl0k' Souchet 17 April 2017
...
static const wchar_t kAFLWhitelistPath[] =
  LR"(syzygy\instrument\test_data\afl-whitelist-good.json)";

static const wchar_t kAFLBlacklistPath[] =
  LR"(syzygy\instrument\test_data\afl-blacklist-good.json)";

static const wchar_t kAFLBadConfigPath[] =
  LR"(syzygy\instrument\test_data\afl-config-bad.json)";
...
TEST_F(AFLInstrumenterTest, ParseMinimalAFL) {
...
TEST_F(AFLInstrumenterTest, ParseFullAFL) {
  SetUpValidCommandLine();
...
TEST_F(AFLInstrumenterTest, ParseWhitelistModeAFL) {
  SetUpValidCommandLine();
  cmd_line_.AppendSwitchPath(
    "config", testing::GetSrcRelativePath(kAFLWhitelistPath)
  );
  ...
  EXPECT_NE(instrumenter_.target_set_.find("fuzzme"), instrumenter_.target_set_.end());
  EXPECT_NE(instrumenter_.target_set_.find("pattern1"), instrumenter_.target_set_.end());
  EXPECT_NE(instrumenter_.target_set_.find("_pattern2"), instrumenter_.target_set_.end());
  EXPECT_NE(instrumenter_.target_set_.find("Unused::M"), instrumenter_.target_set_.end());
}

TEST_F(AFLInstrumenterTest, ParseBlacklistModeAFL) {
  SetUpValidCommandLine();
  cmd_line_.AppendSwitchPath(
    "config",
...
TEST_F(AFLInstrumenterTest, ParseWrongConfigModeAFL) {
  SetUpValidCommandLine();
  cmd_line_.AppendSwitchPath(
    "config",
